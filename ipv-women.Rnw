...
\begin{footnotesize}
<<splines-pretty.print.convergence, results="asis", echo = FALSE>>=
...
\begin{footnotesize}
<<splines-verify-alignment, results="asis", echo = FALSE>>=
library(xtable)
...
<<dual-plot, fig.cap = paste0("Left: Predicted probabilities for a woman to be exposed to IPV conditioned on the attitudes in the couple and the attitudes of the other women in the local area. Credible intervals corresponding to 95\\% of the parameter samples. Right: Lower bounds of the credible intervals (95\\%) of the marginal effect of the attitudes of the other women in the local area on the probability of IPV conditioned on the attitudes in the couple. For couples where no one justifies IPV, the effect of the attitudes of the neighbourhood becomes 'statistically significant' when the proportion of women in the neighbourhood who thinks IPV can be justified is above ", round(marginal.effect.positive.at[1], 2), ". The other types of couples are less susceptible to the attitudes of the women in the neighbourhood, but even in couples where both the husband and the wife thinks IPV can be justified, the attitudes of the neighbourhood have a measurable effect on IPV when the proportion of women in the neighbourhood who thinks IPV can be justified is above ", round(marginal.effect.positive.at[2], 2), ". n = ", nrow(ipv.mm), "."), fig.width = 5, fig.height = 3.3, dpi = 300>>=
...
plot(y = results.marginal[["None justifys IPV"]]["2.5%",],
     ...
     ylab = "Marginal effect on IPV (lower bound)",
     type = "l", col = "white", ylim = c(min(unlist(results.marginal)), 0.16),
     main = "Marginal effect (lower bound)",
     lwd = 1)
grid()
my.names <- c("None justifys IPV", attitudes)
for(i in 1:length(my.names)){
...
Do a similar analysis with the Gender Development Index instead of the attitudes of the women in the neighbourhood. The same dataset can be used.

<<splines-data-preparation-gdi, engine='python', results = 'markup', cache = TRUE>>=
...
my_tab_3 = unique_rows.iloc[:5, np.hstack((df.columns.get_loc("gdi"), range(len(unique_rows.columns)-(num_knots + 1), len(unique_rows.columns))))]
df.to_csv("ipv-women-gdi-model_matrix.csv", index=False)
@ 

<<splines-print.sorted.table-gdi, results='asis', echo = TRUE, cache = TRUE>>=
my.tab <- py$my_tab_3
library(xtable)
print(xtable(my.tab, caption = "GDI and some of the columns that will replace it in the model. These rows represents the right most part of figure \\vref{fig:splines-data-preparation-gdi}; at 0.98 the last curve is at value 0.5927 and the second curve is at value 0.3720.", digits = 4, label = "tab:gdi"), booktabs = TRUE)
@

<<splines-fit-model-gdi, engine = "python", results = "markup", cache = TRUE>>=
import pymc as pm
coords={
    "couple_attitude_dims" : ["Husband justifies IPV", "Wife justifies IPV", "Both justify IPV"],
    "education_dims" : ["Incomplete primary", "Complete primary", "Incomplete secondary", "Complete secondary", "Higher"],
...
print(xtable(table(my.dt.clean$country)), tabular.environment="longtable", floating = FALSE, booktabs = TRUE)
@ 
...
print(xtable(data.frame(Country = names(viol.tab), n = as.vector(viol.tab))), tabular.environment="longtable", floating = FALSE, booktabs = TRUE)
@ 
