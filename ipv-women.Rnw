...
<<load.data.from.scratch, eval = FALSE>>=
library(devtools)
...
## For clusters with missing longitude and/or latitude, use the GDL code that corresponds to the "total" version for the country.
## Ignore Afghanistan since that country has been dealt with the RegionID
my.temp.set.2b <- unique(my.dt[which(is.na(lon) & country.code.ISO.3166.alpha.3 != 4), list(ClusterID, year.of.interview, GDLCODE = paste0(iso.3166$String.code[match(country.code.ISO.3166.alpha.3, iso.3166$numeric)], "t")), ])

## If a cluster is in both sets, keep it only on the set with Sub-National data
## these <- as.vector(na.omit(match(my.temp.set.2$ClusterID, my.temp.set.2b$ClusterID)))
## But `these` is empty, so rbind() is safe

my.temp.set.2.all <- rbind(my.temp.set.2, my.temp.set.2b)

setkey(my.temp.set.2.all, GDLCODE, year.of.interview)
setkey(gdi.long, GDLCODE, year.of.interview)
my.temp.set.3 <- merge(my.temp.set.2.all, gdi.long)
...
my.dt.temp <- my.dt.temp[which(country %in% c("HND", "MDA", "MOZ") == FALSE), ]
...
my.dt.clean <- my.dt.temp[which(is.na(beat01_couple) == FALSE |
                          ...
                          list(viol01_wife, beat01_couple, v149, edudeviation, decide2_purchases, decide2_health, gdi_s, gni_s, viol01_cluster, justifyIPV_cluster, iwi2, country, cluster1, ln_numb_of_children, lnage_wife, couple.is.illiterate, lon, lat, year.of.interview), ]
...
save(gdi.long, file = "gdi.long.RData")
save(gni.long, file = "gni.long.RData")

## clean up
rm(my.dt, my.dt.temp, my.temp.set.4, my.temp.set.3, my.temp.set.2, my.temp.set, gdi, gni, gdi.clean, gni.clean, gdi.long, gni.long, my.cols, my.map, unique.pts.clean, unique.pts, indices, gdl.polygons, gdl.classification, gdl.codes, sf_points, education.levels.ordered, Afghanistan.Region.ID.from.dhs, afghanistan, afghanistan.level1.projected, afghanistan.coordinates)
gc(verbose = FALSE)
