...
## convert gdi.clean from wide to lon
...
setkey(my.temp.set, GDLCODE, year.of.interview)
setkey(gdi.long, GDLCODE, year.of.interview)
my.temp.set.3 <- merge(my.temp.set, gdi.long)
...
## TODO: Use Subnational GNI and GDI in India too, use Björns work to match Indian "countries" with GDLCODEs
...
## non-missing on lon, lat, but justifyIPV_cluster does not required
...
<<splines-show-the-model-matrix, engine='python', cache = TRUE, results = "markup">>=
df.columns
@ 
