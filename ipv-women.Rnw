...
marginal.effect.positive.at <- sapply(names(results), function(attitude){
    index.2 <- head(which(results[[attitude]]["2.5%",] >= 0), 1)
    index.1 <- index.2 - 1
    x.2 <- interesting.points[["justifyIPV_cluster"]][index.2]
    x.1 <- interesting.points[["justifyIPV_cluster"]][index.1]
    y.1 <- results[[attitude]]["2.5%", ][index.1]
    y.2 <- results[[attitude]]["2.5%", ][index.2]
...
par(mar = c(4, 5, 1, 14))
plot(y = results[["None justifys IPV"]]["2.5%",],
     x = interesting.points[["justifyIPV_cluster"]],
     xlab = "Neighbourhood justification of IPV",
     ylab = "Marginal effect on IPV (lower bound)",
     type = "l", col = "white", ylim = c(min(unlist(results)), 0.16),
     lwd = 1)
grid()
my.names <- c("None justifys IPV", attitudes)
for(i in 1:length(my.names)){
