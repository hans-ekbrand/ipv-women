...
required.packages <- c("lme4", "data.table", "xtable", "effects", "parallel", "future", "doFuture", "foreach", "sf", "terra", "parallelly", "cooltools", "raster", "formatR", "lwgeom", "autoFRK", "rnaturalearthdata", "rnaturalearth", "spatstat.geom", "spatstat.random", "reticulate", "devtools", "car")
...
## No, use coordinate data if it exists.
...
## Now, add those clusters for which we do not have any lon, lat
my.no.lon.lat.set <- unique(my.dt[which(is.na(lon) & ClusterID %in% my.temp.set.2$ClusterID == FALSE), list(ClusterID, year.of.interview, GDLCODE = paste0(globallivingconditions::iso.3166$String.code[match(country.code.ISO.3166.alpha.3, globallivingconditions::iso.3166$numeric)], "t")), ])
