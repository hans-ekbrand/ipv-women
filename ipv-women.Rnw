...
def rename_columns(col_name, dummy_vars):
    for var in dummy_vars:
        if col_name.startswith(var + "_"):
            ...
            return f"('{var}', '{level}')"
    return col_name  # Leave unchanged if it's not a dummy variable
...
<<splines-fit-model-12, engine = "python", cache = TRUE, echo = TRUE, eval = FALSE>>=
import pickle
import pymc.sampling.jax
import arviz
...
Make a table over the countries
