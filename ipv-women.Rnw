...
The only remaining problems with missing data is why Telangana and Tamil Nadu lose cases, what is missing there? and why Bangladesh does no turn up in the kriging even when there are 19654 cases in justifyIPV\_cluster. Indonesia (360) is not present in justifyIPV_cluster, perhaps missing coordinates? Then it could be added by the same procedure as Afghanistan via sampling based on the region names, but don't spend time on that just now.
...
these <- which(is.na(couple_attitudes) == FALSE &
                                is.na(viol01_wife) == FALSE &
                                is.na(justifyIPV_cluster) == FALSE &
                                is.na(Education) == FALSE &
                                is.na(edudeviation) == FALSE &
                                is.na(decide_purchases) == FALSE &
                                is.na(decide_health) == FALSE &
                                is.na(iwi2) == FALSE &
                                is.na(ln_numb_of_children) == FALSE &
                                is.na(literate_couple) == FALSE &
