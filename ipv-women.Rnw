...
required.packages <- c("lme4", "data.table", "xtable", "effects", "parallel", "future", "doFuture", "foreach", "sf", "terra", "parallelly", "cooltools", "raster", "formatR", "lwgeom", "autoFRK", "rnaturalearthdata", "rnaturalearth", "spatstat.geom", "spatstat.random", "reticulate", "devtools")
...
my.dt <- download.and.harmonise(
    ...
    waves = list("Bangladesh" = "81", "Congo Democratic Republic" = "61", "Egypt" = "61", "Indonesia" = "71", "Jordan" = "81",
...
## To improve imputations, add country as a variable so that subregions share 
## the same country
## gdi.long[, country := factor(substring(GDLCODE, 1, 3)), ]
## gdi.long$GDLCODE <- factor(gdi.long$GDLCODE)
## imp.model.gdi <- mice::mice(gdi.long, method = "pmm", m = 1, printFlag = FALSE)
...
try:
    ...
    with my_model:
        my_fit_pymc = pm.sample(random_seed=1234, tune=500, draws=200, target_accept=0.8, chains = 2, chain_method='parallel', cores=4, idata_kwargs=dict(log_likelihood=False), postprocessing_backend="cpu", progressbar = False)
