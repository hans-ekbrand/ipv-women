...
required.packages <- c("lme4", "data.table", "xtable", "effects", "parallel", "future", "doFuture", "foreach", "sf", "terra", "parallelly", "cooltools", "raster", "formatR", "lwgeom", "autoFRK", "rnaturalearthdata", "rnaturalearth", "spatstat.geom", "spatstat.random", "readstata13", "mice")
...
my.dt <- download.and.harmonise(
    ...
    waves = list("Afghanistan" = "71", "Angola" = "71", 
                 "Cote d'Ivoire" = "81",
                 "Gabon" = "71",
                 "Jordan" = "74",
                 "Malawi" = "7A",
...
my.dt[, couple.is.illiterate := factor(rep(NA, nrow(my.dt)), levels = c("No one can read", "Only husband can read", "Only wife can read", "Both can read")), ]
my.dt[which(husband.is.illiterate & wife.is.illiterate), couple.is.illiterate := "No one can read", ]
my.dt[which(husband.is.illiterate & (wife.is.illiterate == FALSE)), couple.is.illiterate := "Only wife can read", ]
my.dt[which((husband.is.illiterate == FALSE) & wife.is.illiterate), couple.is.illiterate := "Only husband can read", ]
my.dt[which((husband.is.illiterate == FALSE) & (wife.is.illiterate == FALSE)), couple.is.illiterate := "Both can read", ]
## clean up
...
my.dt.temp[, country := globallivingconditions::iso.3166$String.code[match(country.code.ISO.3166.alpha.3, globallivingconditions::iso.3166$numeric)], ]
...
my.dt.temp[, country := globallivingconditions::iso.3166$String[match(country.code.ISO.3166.alpha.3, globallivingconditions::iso.3166$numeric)], ]
...
my.dt.clean <- my.dt.temp[which(is.na(beat01_couple) == FALSE &
                                ...
                                is.na(couple.is.illiterate) == FALSE &
                                is.na(lnage_wife) == FALSE &                                                                                           is.na(gni_s) == FALSE &
                                is.na(gdi_s) == FALSE
                                ),                                
                          list(viol01_wife, beat01_couple, v149, edudeviation, decide2_purchases, decide2_health, gdi_s, gni_s, viol01_cluster, justifyIPV_cluster, iwi2, country, cluster1, ln_numb_of_children, lnage_wife, couple.is.illiterate), ]
...
## ## remove clusters with only one person in them
## my.tab <- table(my.dt.clean$cluster1)
## remove.these.clusters <- names(my.tab[which(my.tab < 2)])
## my.dt.clean <- my.dt.clean[cluster1 %in% remove.these.clusters == FALSE, ]
...
generate.input.data.for.kriging.f <- function(my.set, my.var, my.clean.dt, projection = "EPSG:3857", my.cell.width = 10000, country.borders.to.ignore = NULL){
    ...
    my.clean.dt[, country := globallivingconditions::iso.3166$String[match(country.code.ISO.3166.alpha.3, globallivingconditions::iso.3166$numeric)], ]

    ## Select the relevant data points 
    my.dt.with.data <- my.clean.dt[ 
        country.code.ISO.3166.alpha.3 %in% globallivingconditions::iso.3166$numeric[match(my.set, globallivingconditions::iso.3166$String)] &
        ! is.na(lon) &
        ! is.na(my.var), ]
...
gray.countries <- world[as.numeric(world$iso_n3_eh) %in%
        globallivingconditions::iso.3166$numeric[match(add.gray.countries, globallivingconditions::iso.3166$String)], ]
...
world <- rnaturalearth::ne_countries(scale = 50, returnclass = "sv")
gray.countries <- world[as.numeric(world$iso_n3_eh) %in% globallivingconditions::iso.3166$numeric[match(add.gray.countries, globallivingconditions::iso.3166$String)], ]
plot(gray.countries, col = "gray80", add = TRUE, border = "darkgreen", lwd = 0.5)
...
Inspect the input data, compare with the reference dataset.

<<spline-some-descriptives, results = "asis">>=
my.tab <- my.dt.clean[which(is.na(viol01_wife) == FALSE &
                            is.na(gdi_s) == FALSE &
                            is.na(gni_s) == FALSE &
                            is.na(beat01_couple) == FALSE
                            ), list(proportion.exposed = round(length(which(viol01_wife))/.N, 3), gdi = round(mean(gdi_s), 3), gni = round(mean(gni_s), 3), N = .N), by = country]

print(xtable(my.tab[order(country), ], caption = "Some descriptives, automatically created dataset, GNI and GDI based on Sub-national estimates by the GDL matched using point in polygon techniques. Number of children includes only living children."), include.rownames = FALSE, tabular.environment = 'longtable', floating = FALSE)

@

<<spline-björns.data, results = "asis">>=
library(readstata13)
my.df <- read.dta13("~/IPV_mw1.dta")

## fix factors
my.df$beat01_couple <- factor(names(attr(my.df, "label.table")$beat01_couple[match(my.df$beat01_couple, attr(my.df, "label.table")$beat01_couple)]), levels = c("No one accept IPV", "Wife accept IPV", "Husband accept IPV", "Wife & husband accept IPV"))

my.df$iliterate_couple <- factor(names(attr(my.df, "label.table")$iliteracy[match(my.df$iliterate_couple, attr(my.df, "label.table")$iliteracy)]), levels = c("Both literate", "Wife iliterate", "Husband iliterate", "Both iliterate"))

my.df$decide2_health <- factor(names(attr(my.df, "label.table")$decide2_health[match(my.df$decide2_health, attr(my.df, "label.table")$decide2_health)]), levels = c("Health - joint decisions", "Health - wife decides", "Health - husband decides", "Health - don't agree"))

my.df$decide2_purchases <- factor(names(attr(my.df, "label.table")$decide2_purchases[match(my.df$decide2_purchases, attr(my.df, "label.table")$decide2_purchases)]), levels = c("Purchases - joint decisions", "Purchases - wife decides", "Purchases - husband decides", "Purchases - don't agree"))

my.df$edudeviation <- factor(names(attr(my.df, "label.table")$edudeviation[match(my.df$edudeviation, attr(my.df, "label.table")$edudeviation)]), levels = c("No difference", "Wife has higher edu.", "Husband has higher edu."))

## pre-emptively exclude cases with missing data. This is useful since we *after* removing these also remove clusters with less than 3 members.
my.df.clean <- data.table(my.df[which(is.na(my.df$viol01_wife) == FALSE &
                                      is.na(my.df$iliterate_couple) == FALSE &
                                      is.na(my.df$ln_numb_of_children) == FALSE), c("d106", "d107", "d108", "viol01_wife", "beat01_couple", "exposedIPV_cluster", "justifyIPV_cluster", "decide2_health", "decide2_purchases", "v149", "edudeviation", "iliterate_couple", "iwi2", "lnage_wife", "ln_numb_of_children", "gdi_s", "bni_s", "country", "cluster1")])
for(var in c("country", "cluster1")){
    my.df.clean[[var]] <- factor(my.df.clean[[var]])
}
## Make proper booleans
for(var in c("d106", "d107", "d108")){
    my.df.clean[[var]] <- ifelse(my.df.clean[[var]] == "yes", TRUE, FALSE)
}
for(var in c("viol01_wife")){
    my.df.clean[[var]] <- ifelse(my.df.clean[[var]] == 1, TRUE, FALSE)
}
my.tab <- my.df.clean[which(is.na(viol01_wife) == FALSE), list(proportion.exposed = round(length(which(viol01_wife))/.N, 3), gdi = round(mean(gdi_s), 3), gni = round(mean(bni_s), 3), N = .N), by = country]
print(xtable(my.tab[order(country), ], caption = "Some descriptives from the reference data set."), include.rownames = FALSE, tabular.environment = 'longtable', floating = FALSE)
@ 
...
dummy_vars = ["beat01_couple", "decide2_health", "decide2_purchases", "edudeviation", "v149", "couple.is.illiterate"]
@
...
coords={
    ...
    "literate_couple_dims" : ["Only husband can read", "Only wife can read", "No one can read"],
...
with my_model:
    interaction_terms = pm.Normal("interaction_terms", mu=0, sigma=5, dims = ["couple_attitude_dims", "splines_dims"])
@
...
with my_model:
    ...
    decide_health = pm.Normal("decide_health", mu=0, sigma=5, dims = "decide_health_dims")
    decide_purchase = pm.Normal("decide_purchase", mu=0, sigma=5, dims = "decide_purchase_dims")
    edudeviation = pm.Normal("edudeviation", mu=0, sigma=5, dims = "edudeviation_dims")
    illiterate_couple = pm.Normal("illiterate_couple", mu=0, sigma=5, dims = "literate_couple_dims")
@
...
with my_model:
    ...
    gdi = pm.Normal("gdi", mu=0, sigma=5)
    gni = pm.Normal("gni", mu=0, sigma=5)
@
...
with my_model:
    ...
    X_only_husband_can_read = pm.Data("X_only_husband_can_read", df["couple.is.illiterate_Only husband can read"])
    X_only_wife_can_read = pm.Data("X_only_wife_can_read", df["couple.is.illiterate_Only wife can read"])
    X_no_one_can_read = pm.Data("X_no_one_can_read", df["couple.is.illiterate_No one can read"])
...
with my_model:
    theta_as_logit = Intercept + \
      ...
      X_only_husband_can_read * illiterate_couple[0] + \
      X_only_wife_can_read * illiterate_couple[1] + \
      X_no_one_can_read * illiterate_couple[2] + \
      X_iwi * iwi2 + \
...
import pymc.sampling.jax
with my_model:
    my_fit_pymc = pm.sampling.jax.sample_numpyro_nuts(random_seed=1234, tune=100, draws=500, target_accept=0.80, chains = 2, chain_method='parallel', progressbar = False, idata_kwargs=dict(log_likelihood=False), postprocessing_backend="cpu")
...
dummies.2 <- cbind(sapply(c("couple.is.illiterate_No one can read",
    "couple.is.illiterate_Only husband can read", "couple.is.illiterate_Only wife can read"), function (dummy){
        car::recode(ipv.mm[[dummy]], recodes = my.recodes)        
    }))
...
my.mm <- cbind(Intercept = rep(1, nrow(ipv.mm)), couple_attitudes, dummies.1, as.is.1, dummies.2, interaction_terms, as.is.2, splines)
...
Verify that the parameters and the values are aligned.

% \scalebox{.55}{\parbox{\textwidth}{

<<spline-verify-alignment, results="asis", echo = FALSE>>=
library(xtable)
print(xtable(data.frame(Parameters = dimnames(params.matrix)[[2]], Values = dimnames(mms[[1]])[[2]])), booktabs = TRUE)
@
