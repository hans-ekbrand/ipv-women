...
required.packages <- c("lme4", "data.table", "xtable", "effects", "parallel", "future", "doFuture", "foreach", "sf", "terra", "parallelly", "cooltools", "raster", "formatR", "lwgeom", "autoFRK", "rnaturalearthdata", "rnaturalearth", "spatstat.geom", "spatstat.random", "reticulate", "devtools", "mice")
...
The only remaining problems with missing data is why Telangana and Tamil Nadu lose cases, what is missing there? and why Bangladesh does no turn up in the kriging even when there are 19654 cases in justifyIPV\_cluster. Indonesia (360) is not present in justifyIPV\_cluster, perhaps missing coordinates? Then it could be added by the same procedure as Afghanistan via sampling based on the region names, but don't spend time on that just now.
