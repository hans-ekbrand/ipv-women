...
n.surveys.exposure.asia <- length(unique(sapply(strsplit(as.character(
    my.dt$cluster1), ".", fixed = TRUE), function(x) {
...
n.surveys.attitudes.asia <- length(unique(sapply(strsplit(as.character(my.dt$cluster1), ".", fixed = TRUE), function(x) { paste(x[c(1, 3)], collapse = ".") })))
...
n.persons.attitudes.asia <- my.dt.clean[country %in% countries.codes.asia, length(which(is.na(woman.thinks.IPV.can.be.justified) == FALSE)), ]
