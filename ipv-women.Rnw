...
try:
    ...
    with my_model:
        my_fit_pymc = pm.sampling_jax.sample_numpyro_nuts(random_seed=1234, tune=10, draws=40, batches=1, target_accept=0.8, chains = 2, chain_method='parallel', idata_kwargs=dict(log_likelihood=False), postprocessing_backend="cpu", progressbar = False)
