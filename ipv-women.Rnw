...
plot(y = results[["None justifys IPV"]]["2.5%",],
     ...
     type = "l", col = "white", ylim = c(min(unlist(results)), 0.11),
     lwd = 1)
grid()
my.names <- c("None justifys IPV", attitudes)
for(i in 1:length(my.names)){
...
## Add some hints.
for(i in 1:4){
    segments(x0=marginal.effect.positive.at[i], y0=-0.01, y1=0.01, col = my.cols[i], lwd = 3)
    text(x=marginal.effect.positive.at[i], y = 0.0125,
         labels=round(marginal.effect.positive.at[i], 2), col = my.cols[i], adj = 0.5)    
}
