...
my.dt <- download.and.harmonise(
    ...
    waves = list("Bangladesh" = "81", "Congo" = "61", "Congo Democratic Republic" = "61", "Philippines" = "82",
                 "Peru" = "6I", "Yemen" = "61", "Albania" = "71", "Namibia" = "61", "Lesotho" = "81",
                 "Indonesia" = "71", "Kyrgyz Republic" = "61", "Senegal" = "8B",
        "Afghanistan" = "71", "Angola" = "71", "Armenia" = "72", "Benin" = "71", "Burkina Faso" = "81", 
                 ...
                 "Jordan" = "74", "Kenya" = "72", "Liberia" = "7A", "Madagascar" = "81", "Maldives" = "71",
...
my.dt.temp[, country := factor(globallivingconditions::iso.3166$String.code[match(country.code.ISO.3166.alpha.3, globallivingconditions::iso.3166$numeric)]), ]
...
## Guinea has only missing,
## Mozambique has only non exposed, can't be right.

my.dt.temp <- my.dt.temp[which(country %in% c("HND", "MDA", "MOZ", "GIN") == FALSE), ]

## Use the string instead of the digit code, so we get more informative labels
my.dt.temp[, country := factor(globallivingconditions::iso.3166$String[match(country.code.ISO.3166.alpha.3, globallivingconditions::iso.3166$numeric)]), ]
...
my.dt.clean <- my.dt.temp[which(is.na(couple_attitudes) == FALSE &
...
## Logg number of cases lost here
globallivingconditions::my.log.f(paste("Number of cases lost due to missing data in my.dt.temp:", nrow(my.dt.temp) - nrow(my.dt.clean)))
...
## Treat the states of India as countries (note that they already have separate values for GDI and GNI, the country change only affects the random intercept).
these <- which(my.dt.clean$country == "India")
## make country a character vector so that we can manipulate it and add new values
my.dt.clean$country <- as.character(my.dt.clean$country)
if(length(these) > 0){
...
n.local.areas.attitudes.asia <- my.dt[country %in% countries.codes.asia &
  is.na(couple_attitudes) == FALSE, length(unique(ClusterID)), ]
