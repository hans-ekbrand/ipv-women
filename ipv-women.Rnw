...
generate.input.data.for.kriging.f <- function(my.set, my.var, my.clean.dt, projection = "EPSG:3857", my.cell.width = 10000, country.borders.to.ignore = NULL){
    ## Add country to the data points object, so that we easily implement different sampling strategies for different countries.
    ## India has more points than we can make proper use of. Possibly also Burundi and Rwanda
    ...
    my.dt.with.data <- my.clean.dt[ 
        ...
        ! is.na(lon) &
        ! is.na(my.var), ] ## only save one observation per coordinate, take the mean value of my.var
    my.dt.with.data[, coordinate := paste(lon, lat, sep = "."), ]
    my.dt <- my.dt.with.data[, list(temp = mean(get(my.var), na.rm = TRUE), lon=unique(lon), country = unique(country), lat = unique(lat)), by = coordinate]
    lon <- my.dt$lon; lat <- my.dt$lat; my.dt[, c("coordinate", "lon", "lat") := NULL, ]; colnames(my.dt)[1] <- my.var
    foo <- suppressWarnings(sp::SpatialPointsDataFrame(coords = matrix(c(lon, lat), ncol = 2), 
                                      data = my.dt,
                                      proj4string=sp::CRS("EPSG:4326")))
    my.proj <- sp::CRS(projection)
    points.with.data <- sp::spTransform(foo, my.proj)
    ...
    ## ## Create grid points, using a slightly enlarged version, twice the cell width
    ## expanded.countries <- terra::buffer(projected.countries, width = 200000)
    ## template <- terra::rast(expanded.countries, resolution = c(my.cell.width, my.cell.width))
    ...
    ## r <- terra::mask(template, expanded.countries)
    r <- terra::mask(template, projected.countries)    
    # Get indices of cells in the polygon
    ...
    ## for countries with many small islands plotting the borders is a bad idea.
    ## If requrested, create a subset of borders to actually plot
    ...
    ## Turns out the grid points do no need to be a geographical object,
    ## any valid coordinates are fine.
    point.data <- data.frame(x = points.with.data@coords[, 1],
                             y = points.with.data@coords[, 2],
                             temp = points.with.data@data[[my.var]],
                             country = points.with.data@data[["country"]])
    colnames(point.data)[3] <- my.var
    
    ## The first two elements are for kriging, the last two elements are useful for
    ## plotting the end results. `borders` is for masking.
    ## Note that they are SpatVector object, so make sure terra is loaded
    ## And terra version 1.7.0 is required for wrap()
    return(list(point.data, my.grid.points, terra::wrap(borders), terra::wrap(borders.to.plot)))
}

@

<<setup.future>>=
setup.future.f <- function(n = NA, type = NA){
    ## n for number of workers.
    ## type must be "multicore" or "multisession", and is ignored if the OS does not support multicore 
    ## (in which case "multisession" is used).
    ## when RAM is limited, "multisession" can be preferable, since the workers then start with an empty R session.

    ## make sure foreach and future are loaded in the callers session
    
    if(is.na(n)){
        ## use Slurm if available, fall back to system.
    ...
    if(parallelly::supportsMulticore()) {
        ...
        if(is.na(type)){
            my.log.f(paste(Sys.time(), "Setting up future using multicore and", n.cores, "cores."))
            future::plan(future::multicore)            
        } else {
            stopifnot(type %in% c("multisession", "multicore"))
            my.log.f(paste(Sys.time(), "Setting up future using multisession and", n.cores, "cores."))            
            future::plan(type)
        }
    } else {
        ## future::plan(future::multisession, gc=TRUE, workers=n.cores)
        my.log.f(paste(Sys.time(), "Setting up future using multisession with", n.cores, "cores."))        
...
kriging.full.pipeline.f <- function(prepared.data, resolution = my.resolution * 1.1){
    ...
    my.f <- function(data, newlocations){
        proper.loc <- data.frame(x = data$x, y = data$y)
        my.log.f(paste(Sys.time(), "Running autoFRK with", nrow(data), "data points starting with", proper.loc$x[1], "and", proper.loc$y[1],
                 "using", nrow(newlocations), "prediction points. First value is", data[[3]][1], "Last value is", data[[3]][nrow(data)]))
        ## foo <- autoFRK::autoFRK(Data=data[[3]], loc = proper.loc, maxK = 1/2 * nrow(data)^(1/2))
        foo <- autoFRK::autoFRK(Data=data[[3]], loc = proper.loc)        
        my.log.f(paste(Sys.time(), "predicting with autoFRK for", nrow(newlocations), " points"))
        bar <- predict(foo, newloc = newlocations)
        bar$pred.value[, 1]
    }

    my.raster <- sapply(1:length(prepared.data), function(j) {

        ## For each island, determine if there are too many points to do kriging on the full set.
        if(nrow(prepared.data[[j]][[1]]) > 90000){

            ## There are two layers of sampling possibly involved here.            
            ## Reduce the local point density
            ## Reduce the global point density.

            ## In Asia, it is enough to reduce locally high point density in India and Nepal.

            ## In Afria both types of reduction would probably be a
            ## good idea (locally high point density exists in "Rwanda")

            ## The easy way out is just to define some African countries as "with abundance of point data".
            
            ## There are more than enough points from India and Nepal. And all African countries too.
            
            countries.with.abundance.of.point.data <- c("India", "Nepal", "Nigeria", "Kenya", "Malawi", "Uganda", "Zambia", "Chad", "Democratic Republic of the Congo", "Benin", "Burundi", "Ethiopia", "Ghana", "Burkina Faso", "United Republic of Tanzania", "Angola", "Cameroon", "Côte d'Ivoire", "Sierra Leone", "Mauritania", "Senegal", "Mozambique", "Mali", "Niger", "Rwanda", "Gabon", "Gambia", "Liberia", "Togo", "Lesotho", "Zimbabwe", "Namibia", "South Africa", "Guyana", "Comoros", "Swaziland")
            
            keep.these <- which(prepared.data[[j]][[1]]$country %in% countries.with.abundance.of.point.data == FALSE)
            if(length(keep.these) > 0){
                my.log.f(paste(Sys.time(), "Countries in this island that will included unconditionally:", paste(unique(prepared.data[[j]][[1]]$country[keep.these]), collapse = ", ")))
            }
            sample.these <- which(prepared.data[[j]][[1]]$country %in% countries.with.abundance.of.point.data)
            if(length(sample.these) > 0){
                my.log.f(paste(Sys.time(), "Countries in this island that will sampled:", paste(unique(prepared.data[[j]][[1]]$country[sample.these]), collapse = ", ")))
            }

            if(length(sample.these) > 0){            
                ## Set a cap on the total amount of points, and fill
                ## the rest from the countries which have more than
                ## enough points.
                max.n <- min(9000 - length(keep.these), length(sample.these))
                sampled <- sample(sample.these, size = max.n)
                my.log.f(paste(Sys.time(), "max.n =", max.n, "length of keep.these:", length(keep.these),
                               "length sample.these:", length(sample.these)))
                my.sets <- setDT(rbind(prepared.data[[j]][[1]][keep.these, ],
                                 prepared.data[[j]][[1]][sampled, ]))
            } else {
                my.sets <- setDT(prepared.data[[j]][[1]])
            }
            my.sets[, country := NULL, ]
            my.log.f(paste(Sys.time(), "class of my.sets", class(my.sets),
                           "colnames of my.sets", paste(colnames(my.sets), collapse = ", ")))
            kriging.results <- my.f(data=my.sets, newlocations = prepared.data[[j]][[2]])
            
    } else {
        ## Remove 'country', to not confuse autoFRK.
        my.data <- prepared.data[[j]][[1]]
        setDT(my.data)
        my.log.f(paste(Sys.time(), "No sampling. Countries in this island: ", paste(unique(my.data$country), collapse = ", ")))
        my.data[, country := NULL, ]
        kriging.results <- my.f(data = my.data, newlocations = prepared.data[[j]][[2]])
    }
        ## Create a single raster, take the mean if bootstrapping was used.
        if(class(kriging.results) == "numeric"){
            suppressWarnings(my.spdf <- sp::SpatialPointsDataFrame(
                coords = prepared.data[[j]][[2]],
                data = data.frame(viol01_cluster = kriging.results),
                proj4string = sp::CRS("EPSG:3857")))
            ## Create an empty grid
            r <- raster::raster(raster::extent(my.spdf), res = resolution, crs = "EPSG:3857")
            ## Fill it with the predictions, and cast into SpatRaster
            raster.mean <- terra::rast(raster::rasterize(my.spdf, r, field = "viol01_cluster", fun = mean))
        } else {
            foo <- sapply(1:length(kriging.results), function(i){
                my.data <- data.frame(viol01_cluster = kriging.results[[i]])
                suppressWarnings(my.spdf <- sp::SpatialPointsDataFrame(
                                                    coords = prepared.data[[j]][[2]],
                                                    data = my.data,
                                                    proj4string = sp::CRS("EPSG:3857")))
                r <- raster::raster(raster::extent(my.spdf), res = resolution, crs = "EPSG:3857")
                return(terra::rast(raster::rasterize(my.spdf, r, field = "viol01_cluster", fun = mean)))
            })
            raster.bar <- terra::rast(foo) # Convert list to RasterStack
            raster.mean <- terra::app(raster.bar, mean) # Calculate mean for each cell
        }
        return(terra::wrap(raster.mean))

    }, simplify = FALSE)

    ## unwrap with rast
    library(terra)
    unpacked.raster <- lapply(my.raster, terra::unwrap)

    ## Compute the union of extents
    common.extent <- Reduce(terra::union, lapply(unpacked.raster, terra::ext))

    # Create a template raster with the common extent and the resolution of the first raster
    template <- terra::rast(ext=common.extent, res=terra::res(unpacked.raster[[1]]), crs=terra::crs(unpacked.raster[[1]]))
    
    ## Extend all rasters to this template
    aligned.rasters <- lapply(unpacked.raster, function(r) terra::resample(r, template, method="bilinear"))
    ...
    ## Concatenate the borders to plot and the borders to use to crop
    borders.to.crop.with <- Reduce(terra::union, sapply(1:length(prepared.data), function(j) { terra::unwrap(prepared.data[[j]][[3]]) }, simplify = FALSE))

    ## Borders to plot is more difficult, since it can contain empty elements
    valid_vectors <- sapply(1:length(prepared.data), function(j) {
        sv <- terra::unwrap(prepared.data[[j]][[4]])
        if (geomtype(sv) == "polygons") return(sv) else return(NULL)
    }, simplify = FALSE)

    ## Remove NULLs
    valid_vectors <- Filter(Negate(is.null), valid_vectors)

    ## Ensure there's something left to union
    if (length(valid_vectors) > 0) {
        borders.to.plot <- Reduce(terra::union, valid_vectors)
    } else {
        borders.to.plot <- NULL  # Or handle the empty case as needed
    }
    
    cropped.raster <- terra::mask(raster.long.lat, borders.to.crop.with)

    return(list(cropped.raster, borders.to.plot))
...
## world$admin has names, but not proper names accoring to the iso.3166 standard,
